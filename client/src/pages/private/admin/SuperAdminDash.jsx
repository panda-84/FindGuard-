...
export default function SuperAdminDash() {
  ...
  const loadAll = async () => {
    setLoading(true);
    try {
      const [c, g, b, u] = await Promise.all([
        callApi("GET", "/companies/all"),
        callApi("GET", "/guards/all"),
        callApi("GET", "/bookings/all"),
        callApi("GET", "/auth/users"),
      ]);
  ...
  return (
    <div className="min-h-screen w-full bg-cover bg-center bg-no-repeat"
      ...
      <div className="min-h-screen w-full bg-black/60 flex">
        ...
        <aside className="hidden md:flex w-56 flex-col bg-black/40 backdrop-blur-md border-r border-purple-500/20">
            ...
            {navItems.map((item) => (
              <button key={item.id} onClick={() => setCurrentPage(item.id)}
                ...
                {item.id === "companies" && pending > 0 && (
                  <span className="ml-auto bg-yellow-500 text-black text-xs font-black px-2 py-0.5 rounded-full">
                    {pending}
                  </span>
                )}
              </button>
        ...
        <div className="md:hidden fixed top-0 left-0 right-0 z-50 bg-black/80 backdrop-blur-md border-b border-purple-500/20 px-4 py-3 flex justify-between items-center">
        ...
        <main className="flex-1 p-4 md:p-8 overflow-y-auto mt-12 md:mt-0">
          ...
          {message && (
            <div className="mb-4 bg-purple-500/20 border border-purple-400/30 rounded-xl p-3 text-center text-white">
              {message}
            </div>
          )}

          {loading && <p className="text-purple-300 text-center mt-10">Loading data...</p>}
          ...
          {!loading && currentPage === "dashboard" && (
            <div className="space-y-6">
              ...
              {pending > 0 && (
                <div className="bg-black/20 backdrop-blur-md rounded-3xl p-6 border border-yellow-500/30">
                  <h2 className="text-lg font-bold text-yellow-400 mb-4">⚠️ Waiting For Approval ({pending})</h2>
                  <div className="space-y-3">
                    {companies.filter(c => c.status === "pending").map((c) => (
                      <div key={c.id} className="flex flex-col sm:flex-row sm:items-center gap-3 bg-yellow-900/20 rounded-xl p-4 border border-yellow-500/20">
                ...
                {bookings.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {bookings.slice(0, 5).map((b) => (
                      <div key={b.id} className="flex flex-col sm:flex-row sm:items-center gap-2 bg-purple-900/20 rounded-xl p-4 border border-purple-500/20">
                        <div className="flex-1">
          ...
          {!loading && currentPage === "companies" && (
            <div className="space-y-4">
              ...
              {companies.map((c) => (
                <div key={c.id} className="bg-black/20 backdrop-blur-md rounded-2xl p-5 border border-purple-500/20 hover:border-purple-400/40 transition">
                  <div className="flex flex-col md:flex-row md:items-center gap-4">
                    <div className="w-14 h-14 bg-purple-700 rounded-full flex items-center justify-center border-2 border-purple-400 flex-shrink-0">
                    ...
                    <div className="flex-1">
                      ...
                      {c.user && <p className="text-purple-400 text-xs mt-1">👤 {c.user?.name} · {c.user?.email}</p>}
                    </div>
                    <div className="flex flex-col items-end gap-2">
                      ...
                      {c.status === "pending" && (
                        <div className="flex gap-2">
                          <button onClick={() => handleApprove(c.id)} className="px-4 py-2 bg-green-600 hover:bg-green-500 text-white text-sm font-bold rounded-lg transition">✅ Approve</button>
                          <button onClick={() => handleReject(c.id)}  className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white text-sm font-bold rounded-lg transition">✕ Reject</button>
                        </div>
                      )}
                      {c.status === "approved" && (
                        <button onClick={() => handleReject(c.id)} className="px-4 py-2 bg-red-600/50 hover:bg-red-600 text-white text-sm font-bold rounded-lg transition">✕ Revoke</button>
                      )}
                      {c.status === "rejected" && (
                        <button onClick={() => handleApprove(c.id)} className="px-4 py-2 bg-green-600/50 hover:bg-green-600 text-white text-sm font-bold rounded-lg transition">✅ Re-Approve</button>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-6">
              ...
              {guards.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {guards.map((g) => (
                    <div key={g.id} className="bg-gradient-to-br from-blue-700 via-blue-900 to-indigo-900 rounded-xl p-5 shadow-lg hover:scale-[1.02] transform transition-all">
                      <div className="w-16 h-16 bg-blue-500 rounded-full flex items-center justify-center mx-auto mb-3 border-2 border-white">
                      ...
                      <div className="flex justify-between mt-3">
                        ...
                        <span className="text-green-300 font-semibold text-sm">Rs {g.price}/hr</span>
                      </div>
          ...
          {!loading && currentPage === "bookings" && (
            <div className="space-y-4">
              ...
              {bookings.length === 0 ? (
                ...
                <div className="space-y-4">
                  {bookings.map((b) => (
                    <div key={b.id} className="bg-black/20 backdrop-blur-md rounded-2xl p-5 border border-purple-500/20 hover:border-purple-400/40 transition">
                      <div className="flex flex-col md:flex-row md:items-center gap-4">
          ...
          {!loading && currentPage === "customers" && (
            <div className="space-y-6">
              ...
              <div className="flex gap-3 flex-wrap">
                <span className="bg-blue-400/20 text-blue-400 px-3 py-1 rounded-full text-sm font-bold">👤 Customers: {customers.length}</span>
                <span className="bg-green-400/20 text-green-400 px-3 py-1 rounded-full text-sm font-bold">🏢 Companies: {companyUsers.length}</span>
              </div>
              {allUsers.length === 0 ? (
                <div className="bg-black/20 backdrop-blur-md rounded-3xl p-10 text-center">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {allUsers.map((u) => (
                    <div key={u.id} className="bg-gradient-to-br from-purple-900/50 to-indigo-900/50 rounded-xl p-5 border border-purple-500/20 hover:border-purple-400/40 transition">
                      <div className="w-14 h-14 bg-purple-700 rounded-full flex items-center justify-center mx-auto mb-3 border-2 border-purple-400">
